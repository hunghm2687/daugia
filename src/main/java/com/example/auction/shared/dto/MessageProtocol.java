...
/**
 * MessageProtocol - Message wrapper for network communication
 * Uses Java Record for immutability and proper accessors
 */
public record MessageProtocol(
  String type,
  Object data,
  ...
  /**
   * Note: Use accessors without 'get' prefix (record feature):
   * - .type() instead of .getType()
   * - .data() instead of .getData()
   * - .status() instead of .getStatus()
   * - .message() instead of .getMessage()
   */
}