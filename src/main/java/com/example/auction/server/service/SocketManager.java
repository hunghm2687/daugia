...
/**
 * SocketManager - Manage socket communication
 * Note: For server-side, we use ClientSession instead
 * This is kept for client-side operations if needed
 */
public class SocketManager {
  private static SocketManager instance;
  private Socket socket;
  ...
  public void connect(String host, int port) throws IOException {
    if (socket == null || socket.isClosed()) {
      ...
      this.out.flush();
      this.in = new ObjectInputStream(socket.getInputStream());
      LoggerUtil.info("✅ Connected to server: " + host + ":" + port);
    }
  }
  ...
  public void close() {
    try {
      ...
      LoggerUtil.error("Error closing socket", e);
    }
  }
}