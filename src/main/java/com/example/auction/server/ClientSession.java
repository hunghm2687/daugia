...
/**
 * ClientSession - Handle one client connection
 */
public class ClientSession implements Runnable {
  ...
  public ClientSession(Socket socket) {
    ...
    try {
      ...
      this.out.flush();
      this.in = new ObjectInputStream(socket.getInputStream());
      LoggerUtil.info("✅ ClientSession created: " + socket.getInetAddress());
    } catch (IOException e) {
      LoggerUtil.error("Error creating ClientSession", e);
      connected = false;
    }
  }
  ...
  public void run() {
    try {
      while (connected) {
        MessageProtocol msg = (MessageProtocol) in.readObject();
        String user = currentUserSession != null ? currentUserSession.getUserName() : "GUEST";
        LoggerUtil.info("📨 Message from " + user + ": " + msg.type());

        handleMessage(msg);
      }
    } catch (EOFException e) {
      LoggerUtil.info("⚠️ Client disconnected: " + socket.getInetAddress());
    } catch (IOException | ClassNotFoundException e) {
      LoggerUtil.error("ClientSession error", e);
    } finally {
      disconnect();
    }
  }
  ...
  private void handleMessage(MessageProtocol msg) {
    try {
      if ("LOGIN".equals(msg.type()) || "SIGNUP".equals(msg.type())) {
        UserDTO userDTO = (UserDTO) msg.data();
        ...
        var handler = HandlerFactory.getHandler(msg.type());
        handler.handle(this, msg, tempSession);
      } else {
        var handler = HandlerFactory.getHandler(msg.type());
        handler.handle(this, msg, currentUserSession);
      }
    } catch (Exception e) {
      LoggerUtil.error("Error handling message: " + msg.type(), e);
      sendMessage(new MessageProtocol(
        msg.type(),
        null,
        "ERROR",
  ...
  public synchronized void sendMessage(MessageProtocol message) {
    try {
      out.writeObject(message);
      out.flush();
      LoggerUtil.debug("📤 Sent: " + message.type());
    } catch (IOException e) {
  ...
  public void disconnect() {
    ...
    try {
      ...
      LoggerUtil.info("❌ Connection closed: " + socket.getInetAddress());

      if (currentUserSession != null) {
        ClientManager.getInstance().removeClient(currentUserSession.getId());
      }
    } catch (IOException e) {
      LoggerUtil.error("Error disconnecting", e);
    }
  }
